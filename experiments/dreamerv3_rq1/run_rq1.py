"""Run RQ1 experiment: DreamerV3 + STARLA vs random testing."""
...
TRAINING_EPISODES = 6
RANDOM_EPISODES = 6
RESULTS_PATH = Path("/workspaces/RL_Testing/experiments/dreamerv3_rq1/results/results.json")
    ...
    if META_PATH.exists():
        try:
            ...
            if backend == "minigrid":
                ...
                # dmc2gym env id format used in DI-engine configs.
...
def _run_starla_and_capture_mutations(runner: StarlaRunner) -> tuple[Any, SearchResult]:
...
def main() -> None:
    ...
    print(f"[RQ1] env={env_id}, oracle={fault_oracle.__class__.__name__}")
    ...
    random_seed_episodes = _collect_agent_episodes(agent, env_id, RANDOM_EPISODES, deterministic=False, seed=SEED + 10_000)
    ...
    report, raw_result = _run_starla_and_capture_mutations(runner)
    ...
    print(f"[RQ1] results saved to: {RESULTS_PATH}")
