...
# pip install mujoco
#
"""Run four STARLA/SAM experiments on MuJoCo HalfCheetah with rho sweep and feasibility report."""
...
from experiments_mujoco_sam.train_dreamerv3_halfcheetah import load_dreamerv3_halfcheetah
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run STARLA/SAM four-experiment feasibility suite (HalfCheetah).")
    parser.add_argument(
        "--seeds",
        type=str,
        default="42",
...
def _run_starla(
    runner: StarlaRunner,
    sam_mutator: SAMGuidedMutatorMujoco | None,
...
def _run_single_experiment(
    *,
    ...
    sam_mutator: SAMGuidedMutatorMujoco | None = None
    if mode == "sam":
        if sam_rho is None:
...
def main() -> None:
    ...
    for seed in seeds:
        ...
        try:
            policy, world_model = load_dreamerv3_halfcheetah(seed=seed)
        except Exception as err:
            raise RuntimeError(
                "Failed to load DreamerV3 checkpoint. "
                "Please ensure checkpoints exist and dependencies are installed. "
                "Try: pip install gymnasium[mujoco] mujoco transformers tensorboardX easydict"
            ) from err
