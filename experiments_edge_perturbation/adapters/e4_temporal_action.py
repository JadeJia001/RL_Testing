"""E4: Temporal Action Perturbation (execution channel).
...
Three variants:
  A (delay)  — execute an action from *d* steps ago.
  B (freeze) — repeat the previous action at randomly selected steps.
  C (smooth) — low-pass filter: ``a'_t = alpha * a_{t-1} + (1 - alpha) * a_t``.

Reference
---------
Tessler et al., "Action Robust Reinforcement Learning and Applications in
Continuous Control" (ICML 2019).  arXiv:1901.09184
Adapted from https://github.com/tesslerc/ActionRobustRL — their PR-MDP and
NR-MDP adversarial action formulations.
"""
...
class TemporalActionAgent:
    """Wrapper agent that applies temporal action perturbation.

    This wraps an existing agent (conforming to STARLA ``AgentProtocol``)
    and corrupts the *action* returned by ``predict``.  It does *not* touch
    the observation or the genetic-transform path.
    ...
    def predict(self, obs: Any, deterministic: bool = True) -> tuple[Any, dict[str, Any]]:
        ...
        if self.variant == "delay":
            ...
            if len(self._action_history) >= 2:
                ...
                curr = action
                # For discrete actions: probabilistic rounding
    ...
    def __getattr__(self, name: str) -> Any:
        """Forward any attribute not on this wrapper to the inner agent."""
        # Avoid infinite recursion for attributes looked up during __init__
        if name in ("inner",):
