"""Run edge-perturbation experiments: baseline + E1(SAM) + E2(Jacobian) + E3(Uncertainty) + E4(Temporal).
...
# ---------------------------------------------------------------------------
# Path setup (same pattern as experiments_sam_mountaincar)
# ---------------------------------------------------------------------------
...
from experiments_edge_perturbation.adapters.e2_jacobian_mutation import JacobianGuidedMutator
from experiments_edge_perturbation.adapters.e3_uncertainty_mutation import UncertaintyGuidedMutator
from experiments_edge_perturbation.adapters.e4_temporal_action import TemporalActionAgent
...
# Constants
# ---------------------------------------------------------------------------
DEFAULT_RESULTS_DIR = Path("/workspaces/RL_Testing/experiments_edge_perturbation/results")
...
# ---------------------------------------------------------------------------
# Fault oracle (reused from SAM experiment)
# ---------------------------------------------------------------------------
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run edge-perturbation experiments (baseline + E1–E4).")
    ...
    parser.add_argument(
        "--objective-thresholds", type=str, default="180.0,0.8,0.8",
    ...
    parser.add_argument("--e2-epsilon", type=float, default=0.05, help="Jacobian epsilon for E2")
    parser.add_argument("--e2-top-k", type=int, default=3, help="Jacobian top-k for E2")
    parser.add_argument("--e3-epsilon", type=float, default=0.05, help="Uncertainty epsilon for E3")
    parser.add_argument("--e3-n-samples", type=int, default=5, help="Uncertainty n_samples for E3")
...
def _collect_agent_episodes(
    ...
    if len(episodes) < n:
        raise RuntimeError(
            f"Unable to collect enough valid episodes: needed={n}, got={len(episodes)}"
        )
    return episodes
...
# ---------------------------------------------------------------------------
# STARLA runner with monkey-patching (same pattern as SAM experiment)
# ---------------------------------------------------------------------------
def _run_starla(
    runner: StarlaRunner,
    mutator: Any | None,
) -> tuple[Any, SearchResult]:
    """Run STARLA with optional transform monkey-patch and safe-mutate guard."""
...
def _build_comparison_report(
    ...
    # Vulnerability profile: fault rate per edge (excluding baseline)
    ...
    # Determine weakest/strongest edge
...
def main() -> None:
    ...
    fault_oracle = GenericGymFaultOracle(env_id)
    results_dir = args.results_dir
    results_dir.mkdir(parents=True, exist_ok=True)

    print(f"[EdgeExp] env={env_id}, seeds={seeds}")
    print(f"[EdgeExp] results_dir={results_dir}")
    ...
    for seed in seeds:
        ...
        print(f"\n[EdgeExp][seed={seed}] Loading DreamerV3 checkpoint ...")
        try:
        ...
        # Patch .heads.get if needed (same as SAM experiment)
        ...
        # ---------------------------------------------------------------
        print(f"[EdgeExp][seed={seed}] Running exp0_baseline ...")
        _set_global_seed(seed)
        ...
        all_results["exp1"] = _run_single_experiment(
            ...
            extra_metrics={
                ...
                "sam_fallback_count": 0,  # updated below
            },
        )
        ...
        print(f"[EdgeExp][seed={seed}] exp1_e1_sam done: "
        ...
        # ---------------------------------------------------------------
        print(f"[EdgeExp][seed={seed}] Running exp2_e2_jacobian ...")
        _set_global_seed(seed)
        ...
        print(f"[EdgeExp][seed={seed}] exp2_e2_jacobian done: "
        ...
        # ---------------------------------------------------------------
        print(f"[EdgeExp][seed={seed}] Running exp3_e3_uncertainty ...")
        _set_global_seed(seed)
        ...
        print(f"[EdgeExp][seed={seed}] exp3_e3_uncertainty done: "
        ...
        # ---------------------------------------------------------------
        print(f"[EdgeExp][seed={seed}] Running exp4_e4_temporal ...")
        _set_global_seed(seed)
        ...
        # E4 wraps the agent; no transform patch
        all_results["exp4"] = _run_single_experiment(
            ...
            fault_oracle=fault_oracle,
            exp_cfg=exp_cfg,
            mutator=None,  # No transform patch for E4
            extra_metrics={
        ...
        print(f"[EdgeExp][seed={seed}] exp4_e4_temporal done: "
        ...
        # Comparison report
        # ---------------------------------------------------------------
        comparison = _build_comparison_report(all_results, env_id, seed)
        _dump_json(results_dir / f"edge_comparison_report_seed_{seed}.json", comparison)
        print(f"\n[EdgeExp][seed={seed}] === Vulnerability Profile ===")
        ...
        print(f"[EdgeExp][seed={seed}] Reports saved to {results_dir}")

    print("\n[EdgeExp] All seeds complete.")
