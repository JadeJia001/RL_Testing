"""Run edge-perturbation experiments for MuJoCo HalfCheetah: baseline + E1(SAM) + E2(Jacobian) + E3(Uncertainty) + E4(Temporal).
...
Usage (inside the CPU dev container)::
    ...
    python -m experiments_edge_halfcheetah.run_edge_experiments --seeds 42
"""
...
import experiments_mujoco_sam.train_dreamerv3_halfcheetah as _halfcheetah_mod
# Monkey-patch to force cuda=False on CPU-only container
_orig_build = _halfcheetah_mod._build_halfcheetah_config
def _patched_build():
    cfg, create_cfg = _orig_build()
    cfg.policy.cuda = False
    cfg.world_model.cuda = False
    return cfg, create_cfg
_halfcheetah_mod._build_halfcheetah_config = _patched_build

from experiments_mujoco_sam.train_dreamerv3_halfcheetah import (
    load_dreamerv3_halfcheetah,
    CHECKPOINT_DIR_HALFCHEETAH,
)
from experiments_mujoco_sam.adapters.sam_mutation_mujoco import SAMGuidedMutatorMujoco
from experiments_edge_halfcheetah.adapters.e2_jacobian_mutation import JacobianGuidedMutator
from experiments_edge_halfcheetah.adapters.e3_uncertainty_mutation import UncertaintyGuidedMutator
from experiments_edge_halfcheetah.adapters.e4_temporal_action import TemporalActionAgent
...
# Constants
# ---------------------------------------------------------------------------
DEFAULT_RESULTS_DIR = Path("/workspaces/RL_Testing/experiments_edge_halfcheetah/results")
DREAMERV3_CKPT_DIR = Path(
    os.environ.get("DREAMERV3_HALFCHEETAH_CKPT_DIR", str(CHECKPOINT_DIR_HALFCHEETAH))
)
...
# ---------------------------------------------------------------------------
# Fault oracle — HalfCheetah override
# ---------------------------------------------------------------------------
class HalfCheetahFaultOracle(FaultOracle):
    """Fault oracle for HalfCheetah with reward_threshold=1000.0."""

    def __init__(self) -> None:
        self._max_steps = 1000
        self._reward_threshold = 1000.0
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run HalfCheetah edge-perturbation experiments.")
    ...
    parser.add_argument(
        "--objective-thresholds", type=str, default="1000.0,0.8,0.8",
    ...
    parser.add_argument("--e2-top-k", type=int, default=5)
...
def _normalize_action(action: Any) -> Any:
    """Ensure continuous actions are ndarray(6,)."""
...
def _collect_agent_episodes(
    ...
    """Collect episodes with continuous actions."""
    env = ContinuousGymnasiumEnv(env_id)
    ...
    while len(episodes) < n and attempts < max_attempts:
        ...
        while not done:
            ...
            episode.append((deepcopy(obs), env_action.copy()))
    ...
    if len(episodes) < n:
        raise RuntimeError(
            f"Unable to collect enough valid HalfCheetah episodes: needed={n}, got={len(episodes)}"
        )
    return episodes


def _collect_random_episodes(env_id: str, n: int, seed: int) -> list[Episode]:
    """Collect random-policy episodes with continuous actions."""
    env = gym.make(env_id)
    episodes: list[Episode] = []
    for ep_idx in range(n):
        ...
        while not done:
            ...
            episode.append((deepcopy(obs), action_arr.copy()))
...
def _run_single_experiment(
    *,
    ...
    env_adapter = ContinuousGymnasiumEnv(env_id)
    ...
    random_eval_episodes = _collect_random_episodes(
        env_id, n=max(total_budget_episodes, total_searched), seed=exp_cfg.seed + 20_000,
    )
...
def main() -> None:
    ...
    fault_oracle = HalfCheetahFaultOracle()
    results_dir = args.results_dir
    results_dir.mkdir(parents=True, exist_ok=True)

    print(f"[EdgeExp-HalfCheetah] env={ENV_ID}, seeds={seeds}")
    print(f"[EdgeExp-HalfCheetah] results_dir={results_dir}")
    ...
    for seed in seeds:
        ...
        print(f"\n[EdgeExp-HalfCheetah][seed={seed}] Loading DreamerV3 checkpoint ...")
        try:
            policy, world_model = load_dreamerv3_halfcheetah(
                checkpoint_dir=args.checkpoint_dir, seed=seed,
            )
        except Exception as err:
            raise RuntimeError(
                "Failed to load HalfCheetah DreamerV3 checkpoint. "
        ...
        # ---------------------------------------------------------------
        print(f"[EdgeExp-HalfCheetah][seed={seed}] Running exp0_baseline ...")
        _set_global_seed(seed)
        ...
        print(f"[EdgeExp-HalfCheetah][seed={seed}] exp1_e1_sam done: "
        ...
        # ---------------------------------------------------------------
        print(f"[EdgeExp-HalfCheetah][seed={seed}] Running exp2_e2_jacobian ...")
        _set_global_seed(seed)
        ...
        print(f"[EdgeExp-HalfCheetah][seed={seed}] exp2_e2_jacobian done: "
        ...
        # ---------------------------------------------------------------
        print(f"[EdgeExp-HalfCheetah][seed={seed}] Running exp3_e3_uncertainty ...")
        _set_global_seed(seed)
        ...
        print(f"[EdgeExp-HalfCheetah][seed={seed}] exp3_e3_uncertainty done: "
        ...
        # ---------------------------------------------------------------
        print(f"[EdgeExp-HalfCheetah][seed={seed}] Running exp4_e4_temporal ...")
        _set_global_seed(seed)
        ...
        print(f"[EdgeExp-HalfCheetah][seed={seed}] exp4_e4_temporal done: "
        ...
        print(f"\n[EdgeExp-HalfCheetah][seed={seed}] === Vulnerability Profile ===")
        ...
        print(f"[EdgeExp-HalfCheetah][seed={seed}] Reports saved to {results_dir}")

    print("\n[EdgeExp-HalfCheetah] All seeds complete.")
