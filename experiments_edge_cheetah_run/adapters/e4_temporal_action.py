"""E4: Temporal Action Perturbation (execution channel) — DMControl Cheetah-run (continuous).
...
Actions are continuous ``np.ndarray`` of shape ``(6,)``.

CRITICAL: the ``smooth`` variant returns float ndarray directly — NO rounding.
...
class TemporalActionAgent:
    """Wrapper agent that applies temporal action perturbation (continuous).
    ...
    def predict(self, obs: Any, deterministic: bool = True) -> tuple[Any, dict[str, Any]]:
        ...
        if self.variant == "delay":
            ...
            if len(self._action_history) >= 2:
                ...
                curr = action
                # Continuous actions: return float ndarray, NO rounding
                if isinstance(curr, np.ndarray):
                    smoothed = self.alpha * np.asarray(prev, dtype=np.float64) + (1.0 - self.alpha) * curr.astype(np.float64)
                    self._perturbation_count += 1
                    return smoothed.astype(curr.dtype), meta
                else:
