"""Train and load DI-engine DreamerV3 for DMControl Cheetah-run (pixel, 64x64 RGB)."""
...
# Lazy-register the env class into DI-engine's ENV_REGISTRY.
from experiments_edge_cheetah_run.envs.cheetah_pixel_env import register_cheetah_env
register_cheetah_env()

CHECKPOINT_DIR_CHEETAH_RUN = _ROOT / "experiments_edge_cheetah_run" / "checkpoints_cheetah_run"
POLICY_CKPT_PATH_CHEETAH_RUN = CHECKPOINT_DIR_CHEETAH_RUN / "dreamerv3_policy.pth"
WORLD_MODEL_CKPT_PATH_CHEETAH_RUN = CHECKPOINT_DIR_CHEETAH_RUN / "dreamerv3_world_model.pth"
META_PATH_CHEETAH_RUN = CHECKPOINT_DIR_CHEETAH_RUN / "dreamerv3_meta.json"

MAX_ENV_STEP_DEFAULT = 100_000
SEED_DEFAULT = 0

IMAGE_SIZE = 64


def _build_cheetah_config() -> tuple[EasyDict, EasyDict]:
    _use_cuda = torch.cuda.is_available()
    cfg = EasyDict(
        dict(
            exp_name="dreamerv3_cheetah_run_rgb",
            env=dict(
                env_id="cheetah_dmcontrol_rgb",
                domain="cheetah",
                task="run",
            ...
            world_model=dict(
                ...
                cuda=_use_cuda,
                model=dict(
                    state_size=(3, IMAGE_SIZE, IMAGE_SIZE),
                    obs_type="RGB",
                    action_size=6,
...
def train_dreamerv3_cheetah_run(max_env_step: int = MAX_ENV_STEP_DEFAULT, seed: int = SEED_DEFAULT) -> tuple[Any, Any, str]:
    mbrl_entry_setup, _ = _import_ding_entry_tools()
    cfg, create_cfg = _build_cheetah_config()
    print("[Train] Using Cheetah-run DMControl DreamerV3 config (pixel 64x64).")

    ckpt_dir = CHECKPOINT_DIR_CHEETAH_RUN
    ...
    return policy, world_model, "cheetah_run"


def load_dreamerv3_cheetah_run(
    ...
    if checkpoint_dir is not None:
        ...
        policy_ckpt = Path(policy_ckpt_path) if policy_ckpt_path is not None else POLICY_CKPT_PATH_CHEETAH_RUN
        world_model_ckpt = Path(world_model_ckpt_path) if world_model_ckpt_path is not None else WORLD_MODEL_CKPT_PATH_CHEETAH_RUN

    if not policy_ckpt.exists() or not world_model_ckpt.exists():
        raise FileNotFoundError("Checkpoint files do not exist. Run train_dreamerv3_cheetah_run first.")

    cfg, create_cfg = _build_cheetah_config()
    print("[Load] Restoring Cheetah-run DreamerV3.")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train DreamerV3 on DMControl Cheetah-run (pixel)")
    parser.add_argument("--max-env-step", type=int, default=MAX_ENV_STEP_DEFAULT)
    parser.add_argument("--seed", type=int, default=SEED_DEFAULT)
    args = parser.parse_args()
    print(f"[Main] Start Cheetah-run DreamerV3 training with max_env_step={args.max_env_step}")
    train_dreamerv3_cheetah_run(max_env_step=args.max_env_step, seed=args.seed)
    loaded_policy, loaded_world_model = load_dreamerv3_cheetah_run(seed=args.seed)
