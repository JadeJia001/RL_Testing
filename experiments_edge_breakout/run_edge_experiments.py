"""Run edge-perturbation experiments for Atari Breakout: baseline + E1(SAM) + E2(Jacobian) + E3(Uncertainty) + E4(Temporal).
...
Usage (inside the CPU dev container)::
    ...
    python -m experiments_edge_breakout.run_edge_experiments --seeds 42
"""
...
from experiments_atari_sam.train_dreamerv3_breakout import (
    load_dreamerv3_breakout,
    CHECKPOINT_DIR_BREAKOUT,
    preprocess_ale_rgb_obs_for_dreamer,
)
from experiments_atari_sam.adapters.sam_mutation_atari import SAMGuidedMutatorAtari
from experiments_edge_breakout.adapters.e2_jacobian_mutation import JacobianGuidedMutator
from experiments_edge_breakout.adapters.e3_uncertainty_mutation import UncertaintyGuidedMutator
from experiments_edge_breakout.adapters.e4_temporal_action import TemporalActionAgent
...
# Constants
# ---------------------------------------------------------------------------
DEFAULT_RESULTS_DIR = Path("/workspaces/RL_Testing/experiments_edge_breakout/results")
DREAMERV3_CKPT_DIR = Path(
    os.environ.get("DREAMERV3_BREAKOUT_CKPT_DIR", str(CHECKPOINT_DIR_BREAKOUT))
)
...
# ---------------------------------------------------------------------------
# Fault oracle — Breakout-specific overrides
# ---------------------------------------------------------------------------
class BreakoutFaultOracle(FaultOracle):
    """Fault oracle for Atari Breakout with fixed thresholds.

    ``reward_threshold=15.0`` and ``max_steps=10_000`` per PROMPT spec.
    """

    def __init__(self) -> None:
        self._max_steps = 10_000
        self._reward_threshold = 15.0
...
def _parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Breakout edge-perturbation experiments.")
    ...
    parser.add_argument(
        "--objective-thresholds", type=str, default="15.0,0.8,0.8",
...
def _collect_agent_episodes(
    ...
    """Collect episodes from the agent in Breakout with ALE RGB preprocessing."""
    ...
    while len(episodes) < n and attempts < max_attempts:
        ...
        obs = preprocess_ale_rgb_obs_for_dreamer(obs)
        ...
        while not done:
            ...
            episode.append((obs.copy(), action_int))
            raw_obs, reward, terminated, truncated, _ = env.step(action_int)
            obs = preprocess_ale_rgb_obs_for_dreamer(raw_obs)
    ...
    if len(episodes) < n:
        raise RuntimeError(
            f"Unable to collect enough valid Breakout episodes: needed={n}, got={len(episodes)}"
        )
    return episodes


def _collect_random_episodes(n: int, seed: int) -> list[Episode]:
    """Collect random-policy episodes in Breakout."""
    env = gym.make(ENV_ID, obs_type="rgb")
    episodes: list[Episode] = []
...
# ---------------------------------------------------------------------------
# Preprocessed Gymnasium env adapter for STARLA
# ---------------------------------------------------------------------------
class _PreprocessedBreakoutEnv:
    """Thin wrapper so STARLA's runner can use Breakout with preprocessed obs."""

    def __init__(self) -> None:
        self.env = gym.make(ENV_ID, obs_type="rgb")
    ...
    def step(self, action: Any) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        if self._last_obs is not None:
            self._mem.append((deepcopy(self._last_obs), action_int))
        raw_obs, reward, terminated, truncated, info = self.env.step(action_int)
        obs = preprocess_ale_rgb_obs_for_dreamer(raw_obs)
    ...
    def set_state(self, state: Any) -> Any:
        ...
        if isinstance(state, dict) and "_ale_state" in state:
            ...
            if hasattr(ale_obj, "restoreState"):
                ale_obj.restoreState(state["_ale_state"])
...
def _run_single_experiment(
    *,
    ...
    env_adapter = _PreprocessedBreakoutEnv()
...
def main() -> None:
    ...
    fault_oracle = BreakoutFaultOracle()
    results_dir = args.results_dir
    results_dir.mkdir(parents=True, exist_ok=True)

    print(f"[EdgeExp-Breakout] env={ENV_ID}, seeds={seeds}")
    print(f"[EdgeExp-Breakout] results_dir={results_dir}")
    ...
    for seed in seeds:
        ...
        print(f"\n[EdgeExp-Breakout][seed={seed}] Loading DreamerV3 checkpoint ...")
        try:
            policy, world_model = load_dreamerv3_breakout(
                checkpoint_dir=args.checkpoint_dir, seed=seed,
            )
        except Exception as err:
            raise RuntimeError(
                "Failed to load Breakout DreamerV3 checkpoint. "
        ...
        # ---------------------------------------------------------------
        print(f"[EdgeExp-Breakout][seed={seed}] Running exp0_baseline ...")
        _set_global_seed(seed)
        ...
        print(f"[EdgeExp-Breakout][seed={seed}] exp0_baseline done: "
              f"reward_fault_rate={all_results['exp0']['search_reward_fault_rate']}")

        # ---------------------------------------------------------------
        # Exp 1: E1 SAM (perception channel)
        # ---------------------------------------------------------------
        print(f"[EdgeExp-Breakout][seed={seed}] Running exp1_e1_sam ...")
        _set_global_seed(seed)
        ...
        print(f"[EdgeExp-Breakout][seed={seed}] exp1_e1_sam done: "
        ...
        # ---------------------------------------------------------------
        print(f"[EdgeExp-Breakout][seed={seed}] Running exp2_e2_jacobian ...")
        _set_global_seed(seed)
        ...
        print(f"[EdgeExp-Breakout][seed={seed}] exp2_e2_jacobian done: "
        ...
        # ---------------------------------------------------------------
        print(f"[EdgeExp-Breakout][seed={seed}] Running exp3_e3_uncertainty ...")
        _set_global_seed(seed)
        ...
        print(f"[EdgeExp-Breakout][seed={seed}] exp3_e3_uncertainty done: "
        ...
        # ---------------------------------------------------------------
        print(f"[EdgeExp-Breakout][seed={seed}] Running exp4_e4_temporal ...")
        _set_global_seed(seed)
        ...
        print(f"[EdgeExp-Breakout][seed={seed}] exp4_e4_temporal done: "
        ...
        print(f"\n[EdgeExp-Breakout][seed={seed}] === Vulnerability Profile ===")
        ...
        print(f"[EdgeExp-Breakout][seed={seed}] Reports saved to {results_dir}")

    print("\n[EdgeExp-Breakout] All seeds complete.")
