"""E2: Jacobian-Guided Sparse Perturbation (encoding channel) — Atari Breakout (RGB).
...
class JacobianGuidedMutator:
    ...
    Only the ``top_k`` most influential are perturbed, yielding a sparse
    ...
    max_embed_dims:
        Limit Jacobian computation to the first N embedding dimensions for
        speed.  Full embed_dim can be 256+ which is slow for per-row backward.
    device:
    ...
    def _compute_jacobian(self, obs: Any) -> np.ndarray:
        ...
        For RGB input of shape ``(3, 64, 64)`` the encoder receives
        ``(1, 3, 64, 64)`` and *J* has shape ``(effective_embed_dim, 12288)``.
        """
        ...
        obs_dim = obs_tensor[0].numel()  # 3*64*64 = 12288
    ...
    def _select_sensitive_obs_dims(self, jacobian: np.ndarray, k: int) -> np.ndarray:
        """Return indices of the *k* pixel-channel dims with largest column norms."""
