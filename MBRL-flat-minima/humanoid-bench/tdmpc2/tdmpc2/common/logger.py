...
class Logger:
    ...
    def _print(self, d, category):
        # Handle categories with prefixes like 'eval_episode_1'
        color_category = category
        if category.startswith("eval_episode_"):
            color_category = "eval_episode"
            
        category_color = CAT_TO_COLOR.get(color_category, "white")
        category = colored(category, category_color)
    ...
    def log(self, d, category="train"):
        assert category in CAT_TO_COLOR.keys() or category.startswith("eval_episode_"), f"invalid category: {category}"
        if self._wandb:
            if category in {"train", "eval", "results"} or category.startswith("eval_episode_"):
                xkey = "step"
        ...
        if category == "eval" and self._save_csv:
            keys = ["step"]
            # Check if episode_reward exists, if not try avg_undiscounted_return as a fallback
            if "episode_reward" in d:
                keys.append("episode_reward")
                self._eval.append(np.array([d[keys[0]], d[keys[1]]]))
            elif "avg_undiscounted_return" in d:
                keys.append("avg_undiscounted_return")
                self._eval.append(np.array([d[keys[0]], d[keys[1]]]))
            else:
                # Just log the step if no suitable reward metric is found
                self._eval.append(np.array([d["step"], np.nan]))
                keys.append("reward")  # Placeholder for CSV header
                
            pd.DataFrame(np.array(self._eval)).to_csv(
                self._log_dir / "eval.csv", header=keys, index=None
            )
        if category != "results" and not category.startswith("eval_episode_"):
