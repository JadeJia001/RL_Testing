...
from tdmpc2.sam_tdmpc import SAM
...
class TDMPC2:
    ...
    def __init__(self, cfg):
        ...
        # Use SAM optimizer instead of Adam
        sam_rho = getattr(cfg, 'sam_rho', 0.05)  # Default to 0.05 if not specified
        base_params = [
            {
        ...
        print("Doing SAM based training")
        self.optim = SAM(
            base_params,
            torch.optim.Adam,
            rho=sam_rho,
    ...
    def update(self, buffer):
        ...
        # First forward pass (SAM first step)
        ...
        z_2 = self.model.encode(obs[0], task)
        zs_2[0] = z_2
        consistency_loss_2 = 0
        for t in range(self.cfg.horizon):
            z_2 = self.model.next(z_2, action[t], task)
            consistency_loss_2 += F.mse_loss(z_2, next_z[t]) * self.cfg.rho**t
            zs_2[t + 1] = z_2

        # Predictions for second pass
        _zs_2 = zs_2[:-1]
        qs_2 = self.model.Q(_zs_2, action, task, return_type="all")
        reward_preds_2 = self.model.reward(_zs_2, action, task)

        # Compute losses for second pass
        reward_loss_2, value_loss_2 = 0, 0
        for t in range(self.cfg.horizon):
            reward_loss_2 += (
                math.soft_ce(reward_preds_2[t], reward[t], self.cfg).mean()
                * self.cfg.rho**t
            )
            for q in range(self.cfg.num_q):
                value_loss_2 += (
                    math.soft_ce(qs_2[q][t], td_targets[t], self.cfg).mean()
                    * self.cfg.rho**t
                )
        consistency_loss_2 *= 1 / self.cfg.horizon
        reward_loss_2 *= 1 / self.cfg.horizon
        value_loss_2 *= 1 / (self.cfg.horizon * self.cfg.num_q)
        total_loss_2 = (
            self.cfg.consistency_coef * consistency_loss_2
            + self.cfg.reward_coef * reward_loss_2
            + self.cfg.value_coef * value_loss_2
        )
        ...
        # Apply second step of SAM
        self.optim.second_step()

        # Update policy using the original zs
        pi_loss = self.update_pi(zs.detach(), task)

        # Update target Q-functions
        self.model.soft_update_target_Q()

        # Return training statistics from the first forward pass for consistent logging
        self.model.eval()
        return {
